...
"""Common-budget route-search comparison under this project's physical model."""
...
from common import Model, save, charge
from solve_transport import construct, metrics
...
def scalar(z):
    # This scalar is only an annealing acceptance temperature, never the reported model objective.
...
def mutate(m, plan, rng):
    """One of five shared, physically screened route operations."""
    n=len(plan)
    for _ in range(35):
        kind=rng.randrange(5)
        q=list(plan)
        if kind==0 and n>1:  # execution order
            i,j=rng.sample(range(n),2);q[i],q[j]=q[j],q[i]
        elif kind==1 and n>1:  # move a whole box
        ...
        elif kind==2:  # split a box to a direct mission
        ...
        elif kind==3:  # change machine type or stop order
            i=rng.randrange(n);g,a,o=q[i]
            if rng.random()<.5:
        ...
        elif kind==4 and n>1:  # merge two missions
...
def search(m, method, initial_plan, seed, budget):
    ...
    while time.monotonic()<limit:
        iterations+=1
        if method=='grasp':
            # Randomized constructive restarts are a distinct search family.
...
def refine_timefirst(m, rs, seconds=6.0):
    """Same on-time, completion-first CP-SAT decoder for every route partition."""
...
def run(budget=8.0,seeds=(0,1,2),schedule_seconds=6.0):
    m=Model();root=Path(__file__).resolve().parents[1]/'results'
    ...
    for seed in seeds:
        for method in METHODS:
            ...
            trials.append(rec)
    # Identical CP-SAT refinement for each method's best route partition, plus the existing baseline.
    candidates=[]
    for rec in trials:
