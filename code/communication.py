...
"""Terrain-aware link checks and conservative continuous-segment certificates."""
from common import *
...
def certificate(m,a,b,fixed,threshold):
    """Prove every moving endpoint in [a,b] has an available link to fixed.
    Use maximum distance at endpoints and either 10dB worst-case obstruction,
    or a conservative raster/triangle line-of-sight proof.
    Local tangent distance bound has +0.02% horizontal inflation.
    """
    ...
    if abs(det)<1e-15:
        # Vertical movement: the lower endpoint ray dominates all higher rays.
        ...
        return -1.,'degenerate'
    # Rasterize swept LOS triangle using all touched pixels; plane minimum on
    # each full pixel is a lower bound on all actual rays inside that pixel.
...
def select_relays(m,rs):
    ...
    candidates=[];covers=[]
    # 750m grid within task bounding box, endpoints included. Every height is legal.
    coords=[]
...
def run():
    from solve_joint import run as solve_joint
    solve_joint()
