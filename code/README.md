...
5. `figure/q1.py` 至 `figure/q4.py` 负责分题数据图，`figure/spatial.py` 负责地形、六边形参考时间、空间椭圆、三维与分时航线图，共用 `figure/common.py` 的路径、样式和空间底图；`figure/overview.py` 渲染总体技术路线图。绘图脚本读取 `results/*.json`，每次成组输出至 `figures/` 下独立子目录。`reporting/` 保留提交工作簿导出。`workflow/reproduce.py` 在新目录复制输入、包代码和见证文件，再逐个直接运行包内实现文件；`workflow/promote_fusion.py` 是当前归档方案的显式晋升工具。
...
- **整组论文绘图**：运行 `mountain_flood/figure/generate.py`，自动建立图组子目录并生成 Q1–Q4、空间图及总体技术路线图。也可单独运行 `q1.py`–`q4.py` 或 `spatial.py`。
...
- 当前时限实现以医疗期望时刻和首批截止为硬约束，其余物资的期望时刻计入加权迟到；Q1 的物理公式与单点组批动态规划未因本次时限修订改变。
