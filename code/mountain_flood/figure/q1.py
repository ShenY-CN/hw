...
def plot_typical_elevation_profile(model, data):
    ...
    ax.fill_between(distances, ground_altitude, color="#B8B8B8", alpha=0.65,
                    label="DEM 地面高程")
    ax.plot(distances, flight_altitude, color="#0072B2", linewidth=1.5,
            label="无人机飞行高度")
...
def plot_payload_energy(model, data):
    ...
    for ax, (drone_type, spec) in zip(axes, model.types.items()):
        ...
        ax.axhline(0.8 * spec["E"], color="#555555", linestyle="--", linewidth=1,
                   label="20%返航储备下的可用能量")
        ax.axvline(cap, color="#D55E00", linestyle=":", linewidth=1.2,
...
def plot_objective_solutions():
    ...
    for label, summary in rows:
        ax.scatter(summary["energy"], summary["time"] / 60, s=70, label=label)
...
def plot_return_soc(data, model):
    ...
    ax.axhline(20, color="#D55E00", linestyle="--", label="最低返航储备 20%")
