...
def construct(m,seed,multi=True,earliest=0):
    ...
    # Priority jitter changes packing/order while preserving explicit deadline checks.
...
def improve_schedule(m,rs,seconds=15,earliest=0,start_after=None):
    """CP-SAT schedules fixed routes, jointly assigning identical machines and batteries."""
...
def relocate_candidates(m,rs,limit=3):
    """Generate physical route regroupings by moving one whole box between trips."""
    options=[]
...
def direct_relief_candidates(m,rs):
    """Split a soft-late box into a legal direct trip with each usable type."""
    late=[]
    for r in rs:
        for b,t in r['deliver'].items():
            b=int(b)
